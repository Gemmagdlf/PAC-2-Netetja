...
L'objectiu del present treball serà la netetja i l'anàlisi del dataset per arribar a conclusions sobre la influència de les diferents característiques descrites en ell sobre la supervivència dels individus en l'accident del Titànic, així com la creació d'un model predictiu que en base a aquestes característiques sigui capaç de preveure si l'individu sobreviuría o no a l'accident.
...
Al ser una base de dades no molt amplia decidim mantenir tots els atributs disponibles. La vairable resposta la qual volem intentar predir amb els analisis és la vairable Survived, aixi dons el que volem samber és si amb les altres vairables sóm capacos de predir la supervivencia dels subjectes. 
...
Veiem que la variable Agete valors mancants.  L'únic que considerem necessari respecte la depuració de dades és tractar amb els valors mancants ja que això pot influir en el nostre anàlisis. Per fer-ho adoptem dos criteris diferents en funció de si el valor mancant està en la variable resposta o la variable explicativa.


* Variable Survived és la vairbale resposta, si hi hagues missings,  no els tractaríem, eliminem tupla. 


* Missings vairbales explicatives: com sabem els missings poden tenir diversos origens. En aquest cas considrarem que es es trata d'un cas MAR: valors mancants que depenen de coses que hem observat. Per tant aplicarem la tècnica de Muliple imputation a través de la funcio `mice()`
...
modelo <- randomForest(Survived ~ Pclass + Sex + Age + SibSp + Parch + Fare +Embarked , data=train,  importance=TRUE, ntree=2000)
modelo
